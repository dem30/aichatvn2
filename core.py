...
class FirestoreHandler:
            ...
            async with asyncio.timeout(300):  # Timeout 5 phút
                async with aiosqlite.connect(Config.SQLITE_DB_PATH, timeout=30.0) as conn:
                    ...
                    if specific_collections:
                        ...
                        collections = [coll for coll in collections if coll in Config.PROTECTED_TABLES or coll in Config.SPECIAL_TABLES]
                    else:
                        collections = [coll for coll in collections if coll not in Config.PROTECTED_TABLES]
                    # Luôn bao gồm bảng đặc biệt
                    ...
                    for collection_name in collections:
                        ...
                        # Đồng bộ bản ghi từ Firestore
                        async def fetch_firestore_records():
                            batch = []
                            query = self.db.collection(collection_name)
                            # Nếu bảng rỗng hoặc là bảng đặc biệt/bảo vệ, đồng bộ tất cả bản ghi
                        ...
                        if progress_callback:
                            await progress_callback((processed_collections / total_collections) * 100.0)
    ...
    async def sync_from_sqlite(
        ...
        try:
            ...
            async with asyncio.timeout(300):  # Timeout 5 phút
                async with aiosqlite.connect(Config.SQLITE_DB_PATH, timeout=30.0) as conn:
                    # Lấy thời gian đồng bộ cuối cùng (giữ nguyên)
                    async with conn.execute(
                    ...
                    # Xóa bản ghi đã bị DELETE trên Firestore (giữ nguyên)
                    async with conn.execute(
                    ...
                    # Lấy schema từ SQLite (giữ nguyên)
                    async with conn.execute("SELECT collection_name, fields FROM collection_schemas") as cursor:
                        local_schemas = {
                            row[0]: self._deserialize_schema(row[1])
                            for row in await cursor.fetchall()
                    ...
                    # Lấy schema từ Firestore (giữ nguyên)
                    async def get_firestore_schemas():
                        schemas = {}
                        async for doc in self.db.collection("collection_schemas").stream():
                            doc_data = doc.to_dict()
                    ...
                    processed_tables, synced_records = 0, 0

                    async def fetch_rows(table, last_sync, record_limit, page_size=1000):
                        # Kiểm tra xem bảng có rỗng không (giữ nguyên)
                        ...
                        # Check nếu có cột timestamp (giữ nguyên)
                        async with conn.execute(f'PRAGMA table_info("{table}")') as cursor:
                        ...
                        while True:
                            ...
                            try:
                                ...
                                self.logger.error(f"{username}: Lỗi query cho table {table}: {str(query_error)} (query: {query}, params: {params}). Skip table.")
                                break
                            
                            if record_limit and offset >= record_limit:
                                break

                    # Loop tables và sync (giữ nguyên, nhưng thêm try-except cho batch append/unpack)
                    for table in tables:
                        ...
                        async for rows in fetch_rows(table, last_sync, record_limit):
                            for row in rows:
                                try:
                                    data = {col: row[column_list.index(col)] for col in valid_columns if col in column_list and col != "rowid"}
                                    ...
                                    self.logger.debug(f"{username}: Added to batch: table={table}, doc_id={doc_id}, data_keys={list(data.keys())}")
                                except Exception as row_error:
                                    self.logger.error(f"{username}: Lỗi xử lý row cho table {table}: {str(row_error)} (row: {row}). Skip row.")
                                ...
                                if len(batch) >= batch_size:
                                    try:
                                        ...
                                        self.logger.debug(f"{username}: Synced batch {len(batch)} for {table}, count={batch_count}")
                                    except Exception as batch_error:
                                        self.logger.error(f"{username}: Lỗi sync batch cho {table}: {str(batch_error)}. Skip batch.")
                                    batch.clear()
                        ...
                        if batch:
                            try:
                                ...
                                synced_records += batch_count
                                self.logger.debug(f"{username}: Synced final batch {len(batch)} for {table}, count={batch_count}")
                            except Exception as batch_error:
                                self.logger.error(f"{username}: Lỗi sync final batch cho {table}: {str(batch_error)}")
                            batch.clear()
                        ...
                        processed_tables += 1
                        if progress_callback:
                            await progress_callback((processed_tables / total_tables) * 50.0)

                    # Ghi log tổng (giữ nguyên, nhưng wrap _serialize_value để handle int)
                    if synced_records > 0:
                        query = (
                            "INSERT INTO sync_log (id, table_name, record_id, action, timestamp, details) "
                            "VALUES (?, ?, ?, ?, ?, ?)"
                        ...
                        details_json = self._serialize_value(details)  # Đảm bảo str
                        params = (
                            str(uuid.uuid4()),
                            "ALL_TABLES",
                            None,
                    ...
                    return {"success": f"Đồng bộ {synced_records} bản ghi từ SQLite sang Firestore", "synced_records": synced_records}
